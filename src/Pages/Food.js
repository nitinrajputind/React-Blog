    ...
    <>
      ...
      <div className="TheLatest">
        ...
        <div className="Latest_contanier">
          {Apidata.filter((data) => data.category === "Food" && data.id <= 58).map(
            (item, index) => {
              // console.log(item)
              return (
                <TheLatest
      ...
      {/* style={{width : "710px", border : "2px solid Blue"}} */}
        ...
        </h2>
        {
         ...
         Apidata.filter((data)=>data.category === "Food" ).map((item, index)=>{
      ...
      <div className="Latest_contanier">
        {
          Apidata.filter((data)=>data.id % 2 === 0 && (data.category === "Food" && data.id % 3 !== 0)).map((item,index)=>{
            console.log(item)
            return(
              <TextNews key={index}
              title = {item.title} 
